...
TESTS="IUT1 IUT2 IUT3 XALT FPT1"
...
myxxd -s 16 -c $outcols ref/T801B/$tst.out ref.hex
myxxd -s 16 -c $outcols tmp/$tst.sim sim.hex

# combine files line-by-line
if [ "X$inpcols" != "X0" ];
then
  echo "Converting input to HEX..."
  myxxd -s 16 -c $inpcols inp/inp/$inp inp.hex
